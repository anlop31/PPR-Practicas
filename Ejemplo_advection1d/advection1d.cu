...
//*************************************************
// VERSION OF THE FD UPDATE USING ONLY GLOBAL MEMORY     
// ************************************************
...
//*************************************************
// TILING VERSION  (USING A SHARED MEMORY ARRAY) OF THE FD UPDATE   
// ************************************************
...
int main(int argc, char* argv[]){ 
  ...
  if (argc != 2) {
	  cerr << endl<<"Sintaxis: " << argv[0] << " <Número_puntos_en_X (1000-200000) >" << endl;
	return(-1);
	}	
  ...
  if (err!=cudaSuccess) 
    cout<<"ERROR GETTIN¿G A CUDA DEVICE"<<endl;  
...
while (passed && i<n){
  ...
  if (diff>1.0e-4) { 
    passed=0; 
    cout <<"HIGH ERROR IN POINT "<<i<< ":  "<<diff<<endl;}
    i++;
  } 
...
cout<<endl; 
cout<< "Speedup (T_CPU/T_GPU)= "<<Tcpu/Tgpu<<endl<<endl;
